...
void main()
{
	...
	if (cos_val > 0)
	{
		...
		alpha = clamp(1-exp(-refract_length*2), 0, 1);
	}
	else
	{
		alpha = 1.0;
	...
	// Foam texture will need its own PBR prior to HDR tonemapping, but... let's get it working first
...
//		vec4 foam_color = texture(material.texture_diffuse2, vec2(u, v));
		...
		float foam_alpha = min(foam_color.a, max(f1, f2));
...
//	specular = 10000 * 10000 * specular * scene_L * dot(normal, view_dir);
	specular = vec3(0.0);
...
vec3 directionalLight(DirLight light, vec3 normal, vec3 view_dir)
{
	...
	float roughness = 0.1; // water roughness
	vec3 F0 = vec3(0.02);	// water Fresnel normal incidence
...
vec3 getTexNormal()
{
	...
	for (int i = 0; i < NUM_TEX_WAVES; ++i)
	{
		...
		// Gerstner
		float angle = tex_waves[i].freq*dot(tex_waves[i].wave_dirs, vec2(WorldPos.x, -WorldPos.z)) - tex_waves[i].phase;
...
//		float angle = tex_waves[i].freq*dot(tex_waves[i].wave_dirs, vec2(WorldPos.x, -WorldPos.z)) - tex_waves[i].phase;
//		vec3 tmp;
//		new_norm.x -= k * tex_waves[i].freq * tex_waves[i].wave_dirs.x * amp * cos(angle) * pow((sin(angle)+1)/2, k-1);
//		new_norm.z -= k * tex_waves[i].freq * -tex_waves[i].wave_dirs.y * amp * cos(angle) * pow((sin(angle)+1)/2, k-1);
//		new_norm.y += amp;
	}
