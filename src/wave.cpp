...
Wave::Wave(float amp, float wl, float chop, float dur, glm::vec2 dir, double att_offset) :
    avg_amplitude(amp), avg_wavelength(wl), chop(chop),
    duration(dur), avg_direction(dir), _att_offset(att_offset)
{
    resetWave();
    _reset_time = _reset_time - _att_offset * duration;
    _start_time = _reset_time;
...
void Wave::resetWave()
{
    ...
    _amplitude = float(0.5*avg_amplitude + (float(std::rand())/float(RAND_MAX)) * (1.5*avg_amplitude));  // 10% amplitude variation
...
float Wave::amplitude()
{
    ...
    {
        return _amplitude * float(0.5 - 0.5 * std::cos(3.14159 * (duration - instance_elapsed) / _fade_in_time));
    }
...
std::vector<Wave> makeTexWaves(size_t num_waves)
{
	...
	//{
	//	auto lambda = i * .4 + 0.3;
	//	tx_waves.push_back(Wave(.005 * lambda, lambda, 2, 1600., { 1, 0.5 + pow(-1, i)*i/15. }, 1/16.));
	//}
